...
function DataReviewContent() {
  ...
  const queue = useMemo(() => [
    ...
    { label: "Indicator values pending", count: store.indicatorRuns.filter(run => run.status === "review_pending" && store.indicatorValues.some(item => item.id === run.indicator_id && indicatorPassesFilter(item))).length, sub: "Pending indicator runs, one review unit each", section: "indicators" as Section, icon: Gauge },

...
import { SCORE_POINTS, TEAM_LABEL, initialsOf } from "@/config/assessmentCriteria";
import {
  ...
  <span
    title={
      state.spread === null
        ...
        : `${state.entries.length} ${state.entries.length === 1 ? "entry" : "entries"}, ${state.low} to ${state.high}`
    }
    className={cn(
...
/** Filled square = at least one entry, hollow = nothing recorded. Never a number. */
export const CoverageMark: React.FC<{ state: AssessmentState }> = ({ state }) => (
  <span
    ...
    title={`${ASSESSMENT_FLAG_LABEL[state.flag]}${
      state.entries.length ? ` · ${state.entries.length} entries` : ""
    }`}
    className={cn(
      "inline-block h-3 w-3 rounded-[3px] border",
      state.flag === "not_assessed"
...
export const ContributorMark: React.FC<{ entry: AssessmentEntry; name: string }> = ({ entry, name }) => (
  <span
    title={`${name} (${TEAM_LABEL[entry.team]}) — ${entry.score}${entry.note ? ` · ${entry.note}` : ""}`}
...
/**
 * 1..5 rail. Clicking a point records the current user's entry immediately;
 * clicking the active point again withdraws it. No zero, no negative end.
 */

...
export function NodeFilterBar() {
  ...
  return <div className="flex flex-wrap items-center justify-between gap-3 border-y bg-muted/20 px-4 py-3">
    <div className="flex flex-wrap items-center gap-2">
      ...
      {filter.feedstock && <Badge variant="outline" className="h-7 gap-1 pl-2 pr-1 text-[10px] font-normal">Feedstock: {filter.feedstock}<Button variant="ghost" size="icon" className="h-5 w-5" aria-label="Remove Feedstock filter" onClick={filter.clearFeedstock}><X className="h-3 w-3" /></Button></Badge>}
      {filter.product && <Badge variant="outline" className="h-7 gap-1 pl-2 pr-1 text-[10px] font-normal">Product: {filter.product}<Button variant="ghost" size="icon" className="h-5 w-5" aria-label="Remove Product filter" onClick={filter.clearProduct}><X className="h-3 w-3" /></Button></Badge>}
    </div>

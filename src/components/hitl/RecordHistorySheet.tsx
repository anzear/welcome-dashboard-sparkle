...
import { FIELD_LABELS, groupById, indicatorLabel, useHitlStore, type AuditEntityType, type AuditEntry, type Company, type Group, type HitlRecord, type IndicatorValue, type PaperPatentMatch } from "@/lib/hitlStore";
  ...
  return <>
    {history.length === 0 ? <div className="py-20 text-center text-xs text-muted-foreground">No changes recorded.</div> : <div className="space-y-2">
      {history.map(entry => {
        ...
        const fieldLabel = entry.entity_type === "company" && entry.field ? FIELD_LABELS[entry.field] ?? entry.field : entry.field ?? "record";
        return <div key={entry.id} ref={node => { refs.current[entry.id] = node; }} className={cn("rounded-md border px-3 py-3 transition-colors", highlight === entry.id && "border-primary bg-primary/5")}>
          <div className="mb-2 flex flex-wrap items-center gap-2"><time className="font-mono text-[9px] text-muted-foreground">{format(new Date(entry.timestamp), "dd MMM yyyy, HH:mm:ss")}</time><ActorStamp name={entry.actor} timestamp={entry.timestamp} /><OperationChip operation={entry.operation} /></div>
           <div className="grid gap-2 sm:grid-cols-[100px_1fr_auto]"><code className="font-mono text-[10px] text-muted-foreground">{fieldLabel}</code>{entry.entity_type === "pathway" && entry.field === "group_id" ? <div className="flex items-center gap-2 text-xs"><GroupChip group={typeof entry.prior_value === "string" ? groupById(store.groups, entry.prior_value) : null} /><span>→</span><GroupChip group={typeof entry.new_value === "string" ? groupById(store.groups, entry.new_value) : null} />{typeof entry.new_value === "string" && !groupById(store.groups, entry.new_value) && <code>{entry.new_value}</code>}</div> : <ValueDiff prior_value={entry.prior_value} new_value={entry.new_value} />}<Button size="sm" variant="outline" className="h-6 px-2 text-[10px]" disabled={Boolean(undo)} onClick={() => setConfirm(entry)}>Revert</Button></div>
    ...
    <AlertDialog open={confirm !== null} onOpenChange={open => { if (!open) setConfirm(null); }}>
      <AlertDialogContent>
        <AlertDialogHeader><AlertDialogTitle>{intervening.length ? "This change has been superseded" : `Revert ${confirm?.entity_type === "company" && confirm.field ? FIELD_LABELS[confirm.field] ?? confirm.field : confirm?.field ?? "record"}?`}</AlertDialogTitle><AlertDialogDescription>{intervening.length ? "Later changes will be overwritten by this revert." : <>Revert <code>{confirm?.entity_type === "company" && confirm.field ? FIELD_LABELS[confirm.field] ?? confirm.field : confirm?.field}</code> to its prior value? A new audit entry will be written.</>}</AlertDialogDescription></AlertDialogHeader>

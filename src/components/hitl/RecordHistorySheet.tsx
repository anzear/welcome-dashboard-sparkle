...
const reconfirmTotal = (value: unknown) => Array.isArray(value) ? value.length : 0;
const seenAgainTotal = (value: unknown) => value && typeof value === "object" && "count" in value ? Number((value as { count: number }).count) : 0;
    ...
    {history.length === 0 ? <div className="py-20 text-center text-xs text-muted-foreground">No changes recorded.</div> : <div className="space-y-2">
      {history.map(entry => {
        ...
        return <div key={entry.id} ref={node => { refs.current[entry.id] = node; }} className={cn("rounded-md border px-3 py-3 transition-colors", highlight === entry.id && "border-primary bg-primary/5")}>
          <div className="mb-2 flex flex-wrap items-center gap-2"><time className="font-mono text-[9px] text-muted-foreground">{format(new Date(entry.timestamp), "dd MMM yyyy, HH:mm:ss")}</time><ActorStamp name={entry.actor} timestamp={entry.timestamp} /><OperationChip operation={entry.operation} />{entry.entity_type === "company_node_link" && <Badge variant="outline" className="h-5 whitespace-nowrap px-2 text-[9px] font-normal text-muted-foreground">Node link{entry.node_type ? ` · ${nodeLinkTypeLabel(entry.node_type)}` : ""}{entry.node_value ? ` · ${entry.node_value}` : ""}</Badge>}</div>
           {entry.operation === "reconfirm" || entry.operation === "seen_again" ? <div className="grid gap-2 sm:grid-cols-[100px_1fr]"><code className="font-mono text-[10px] text-muted-foreground">{fieldLabel}</code><span className="text-[10px] text-muted-foreground">{entry.operation === "reconfirm" ? `Re-confirmed by an enrichment run (${reconfirmTotal(entry.new_value)}× in total). The record and its review status are unchanged.` : `Found again by an enrichment run (${seenAgainTotal(entry.new_value)}× in total). Still rejected — nothing was reinstated.`}</span></div> : <div className="grid gap-2 sm:grid-cols-[100px_1fr_auto]"><code className="font-mono text-[10px] text-muted-foreground">{fieldLabel}</code>{entry.entity_type === "pathway" && entry.field === "group_ids" ? <div className="flex flex-wrap items-center gap-2 text-xs"><GroupIdList ids={entry.prior_value} /><span>→</span><GroupIdList ids={entry.new_value} /></div> : <ValueDiff prior_value={entry.field === "method_tag" ? methodTagLabel(entry.prior_value as IndicatorValue["method_tag"]) : entry.prior_value} new_value={entry.field === "method_tag" ? methodTagLabel(entry.new_value as IndicatorValue["method_tag"]) : entry.new_value} />}<Button size="sm" variant="outline" className="h-6 px-2 text-[10px]" disabled={Boolean(undo)} onClick={() => setConfirm(entry)}>Revert</Button></div>}
...
const labels: Record<AuditEntityType, string> = { pathway: "Pathway", group: "Group", company: "Company", paper_match: "Paper match", patent_match: "Patent match", indicator_value: "Indicator value", enrichment_run: "Enrichment run", indicator_run: "Indicator run", bulk_job: "Bulk job", company_node_link: "Company node link" };

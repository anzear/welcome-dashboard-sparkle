...
export function RoleNodeLine({ company, compact = false }: { company: Pick<Company, "role" | "role_nodes">; compact?: boolean }) {
  const ref = roleNode(company);
  return <span className={cn("flex flex-wrap items-center gap-1.5", compact && "text-[10px]")}><CompanyRoleChip role={company.role} /><span><span className="text-muted-foreground">{ref.verb}:</span> {company.role_nodes.length ? company.role_nodes.map(value => <strong key={value} className="text-foreground">{value}{value === company.role_nodes[company.role_nodes.length - 1] ? "" : ", "}</strong>) : <span className="text-muted-foreground">Not set</span>}</span></span>;
}
...
export function CompanyDerivedPathwayList({ company }: { company: Company }) {
  ...
  if (!rows.length) return <p className="py-4 text-xs text-muted-foreground">No pathway contains {ref.positionLabel}: {ref.value || "not set"}.</p>;
...
export function DerivedPathwaysForCompany({ company }: { company: Company }) {
  ...
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><Button variant="link" className="h-auto p-0 text-[10px]" onMouseEnter={() => setOpen(true)}>{count} {count === 1 ? "pathway" : "pathways"}</Button></PopoverTrigger><PopoverContent className="w-[min(92vw,760px)]" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}><p className="mb-2 text-[10px] text-muted-foreground">Pathways containing {ref.positionLabel}: {ref.value}</p><CompanyDerivedPathwayList company={company} /></PopoverContent></Popover>;
}

...
import { computeFit, derivedCompanyPathwayIds, useHitlStore, type Company, type CompanyFit } from "@/lib/hitlStore";
...
export function SecondaryNodes({ company, compact = false }: { company: Pick<Company, "secondary_nodes">; compact?: boolean }) {
  const hasAny = Object.values(company.secondary_nodes).some(Boolean);
  if (!hasAny) return <span className="text-[10px] text-muted-foreground">No secondary data</span>;
  return <div className="flex flex-wrap items-center gap-1"><span className="text-[10px] text-muted-foreground">Also known:</span><NodeChips nodes={company.secondary_nodes} compact={compact} /></div>;
}

export function CompanyDerivedPathwayList({ company }: { company: Company }) {
  const store = useHitlStore(); const ref = roleNode(company);
  const rows = derivedCompanyPathwayIds(company, store.pathways).map(id => store.pathways.find(pathway => pathway.id === id)).filter((pathway): pathway is NonNullable<typeof pathway> => Boolean(pathway)).map(pathway => ({ pathway, fit: computeFit(company, pathway) })).sort((a, b) => ({ exact: 0, strong: 1, broad: 2 }[a.fit.level] - { exact: 0, strong: 1, broad: 2 }[b.fit.level]));

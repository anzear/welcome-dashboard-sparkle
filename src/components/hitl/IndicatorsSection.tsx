...
function CorrectDialog({ itemId, focusJustification, focusSources, onClose }: { itemId: string | null; focusJustification: boolean; focusSources?: boolean; onClose: () => void }) {
  ...
  return <Dialog open={itemId !== null} onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Correct indicator value</DialogTitle><DialogDescription>{item ? <span className="space-y-2"><span className="flex flex-wrap items-center gap-2"><ScopeChip scope={item.scope} /><span className="text-xs font-medium text-foreground">{indicatorLabel(item.indicator_key)}</span></span><TargetRef iv={item} /></span> : "Enter a corrected value."}</DialogDescription></DialogHeader>

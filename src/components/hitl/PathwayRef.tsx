...
export function PathwayRef({ pathwayId, variant, emphasisNode, emphasisNodes, emphasisValues, showId = true, showStatus = true, pathway: suppliedPathway }: {
  ...
  if (variant === "card") return <div className="rounded-md border bg-background p-3 text-left">
    <div className="mb-3 flex items-center justify-between gap-2">
      {showId ? <code className="font-mono text-[10px] font-medium">{pathway.id}</code> : <span />}
      {showStatus && <VisibilityChip state={pathway.visibility.default} />}
    </div>
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    <PopoverTrigger asChild>
      <span className="inline-flex min-w-0 max-w-full cursor-help items-center gap-1.5" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
        {showId && <code className="shrink-0 font-mono text-[10px]">{pathway.id}</code>}

...
export function SplitAddButton({ label, icon: Icon, onClick, items, ariaLabel }: { label: string; icon: LucideIcon; onClick: () => void; items: SplitAddButtonItem[]; ariaLabel: string }) {
  return <div className="inline-flex">
    <Button size="sm" className="h-8 rounded-r-none pr-3 text-[10px]" onClick={onClick}><Icon className="mr-1 h-3 w-3" />{label}</Button>
    <DropdownMenu>
      <DropdownMenuTrigger asChild><Button size="sm" className="h-8 w-8 rounded-l-none border-l border-primary-foreground/20 p-0" aria-label={ariaLabel}><ChevronDown className="h-3.5 w-3.5" /></Button></DropdownMenuTrigger>

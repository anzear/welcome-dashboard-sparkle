...
import { SourcesEditor, SourcesPopover, cleanSources, sourcesValid } from "./IndicatorSources";
import { ScopeChip, TargetRef } from "./IndicatorPrimitives";
import {
  COMPUTED_INDICATOR_ERROR, INDICATORS, METHOD_TAGS, normalizeUnit, unitForStorage, SCOPE_TARGET_KEYS, TARGET_POSITION_LABELS, affectedPathwayIds, emptyIndicatorTarget,
  findIndicatorValue, parseSourceList, sameSources, type IndicatorDefinition, type IndicatorSource, type IndicatorTarget, type IndicatorTargetKey, type IndicatorValue,
...
type RawRow = { indicator: string; feedstock: string; process: string; product: string; application: string; value: string; unit: string; valueDate: string; justification: string; method: string; methodDetail: string; note: string; sources: string };
type BulkRow = RawRow & { key: string; sourceList: IndicatorSource[]; definition: IndicatorDefinition | null; target: IndicatorTarget; errors: string[]; warnings: string[]; duplicate: boolean; existingId: string | null; existingMode: ExistingMode; outcome?: Outcome; recordId?: string; pathwayCount?: number };
...
const emptyRaw = (): RawRow => ({ indicator: "", feedstock: "", process: "", product: "", application: "", value: "", unit: "", valueDate: "", justification: "", method: "", methodDetail: "", note: "", sources: "" });
...
function buildRows(rawRows: RawRow[], existingValues: IndicatorValue[]): { rows: BulkRow[]; duplicates: number } {
  ...
  const rows = rawRows.slice(0, 200).map((raw, index) => {
    ...
    if (definition && !definition.computed) {
      ...
      if (normalizeUnit(definition.key, raw.unit) === null) errors.push(`Unit not accepted: ${raw.unit.trim()}. Accepted: ${definition.units.join(", ")}`);
      if (!raw.value.trim()) warnings.push("No value");
    }
    const sourceList = parseSourceList(raw.sources);
    if (sourceList.length && !sourcesValid(sourceList)) errors.push("Invalid source URL");
    ...
    return { ...raw, key: `${index}-${identity}`, sourceList, definition, target, errors, warnings, duplicate, existingId: existing?.id ?? null, existingMode: "skip" as const };
  });
...
export const downloadBulkIndicatorValuesTemplate = () => {
  const headers = ["Indicator", "Feedstock", "Process", "Product", "Application", "Value", "Unit", "Value date", "Justification", "Method", "Method detail", "Note", "Sources"];
  const example = ["Product price", "", "", "Lactic acid", "", 1650, "EUR/t", "2026-06-30", "Average of three European quotes", "Expert judgement", "Quotes reviewed for comparable grades", "Optional note", "https://www.icis.com/pricing [ICIS 2026] | https://ec.europa.eu/eurostat"];
...
export function BulkAddIndicatorValuesDialog({ open, onClose, onOpenRecord }: { open: boolean; onClose: () => void; onOpenRecord: (id: string) => void }) {
  ...
  const writeRows = () => {
    ...
    const results = rows.map(row => {
      ...
      if (existing) {
        const changes: [keyof IndicatorValue, unknown, unknown][] = [["corrected_value", existing.corrected_value, parsedValue], ["unit", existing.unit, unitForStorage(row.definition.key, row.unit)], ["value_date", existing.value_date, date], ["justification", existing.justification, row.justification.trim()], ["method_tag", existing.method_tag, methodTag], ["method_detail", existing.method_detail, row.methodDetail.trim() || null], ["correction_note", existing.correction_note, note], ["corrected_at", existing.corrected_at, now]];
        if (!sameSources(cleanSources(row.sourceList), existing.sources)) store.recordChange({ entity_type: "indicator_value", entity_id: existing.id, field: "sources", prior_value: existing.sources, new_value: cleanSources(row.sourceList), operation: "update", note });
        ...
        if (existing.status !== "approved") store.recordChange({ entity_type: "indicator_value", entity_id: existing.id, field: "status", prior_value: existing.status, new_value: "approved", operation: "approve", note: "bulk correction" });
      ...
      numericId += 1; const id = `iv-${String(numericId).padStart(3, "0")}`; const record: IndicatorValue = { id, created_at: now, updated_at: now, status_changed_at: now, last_actor: store.currentUser.name, trace_id: null, indicator_key: row.definition.key, scope: row.definition.scope, target: row.target, value: null, unit: unitForStorage(row.definition.key, row.unit), value_date: date, status: pendingReview ? "review_pending" : "approved", corrected_value: parsedValue, correction_note: note, corrected_at: now, justification: row.justification.trim(), method_tag: methodTag, method_detail: row.methodDetail.trim() || null, sources: cleanSources(row.sourceList), overrides: typeof parsedValue === "number" ? [{ id: `ov-${id}-1`, value: parsedValue, unit: unitForStorage(row.definition.key, row.unit), reason: note, entered_by: store.currentUser.name, entered_at: now, superseded_at: null, superseded_by_run_id: null }] : [] };
  ...
  const exportCsv = () => { const values = [["Batch ID", "Indicator", "Target", "Outcome", "Record", "Pathways"], ...rows.map(row => [batchId, row.indicator, SCOPE_TARGET_KEYS[row.definition?.scope ?? "feedstock"].map(key => row.target[key]).filter(Boolean).join(" → "), row.outcome ?? "Invalid", row.recordId ?? "", row.pathwayCount ?? 0])]; const csv = values.map(line => line.map(value => `"${String(value).replace(/"/g, '""')}"`).join(",")).join("\n"); const url = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" })); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "indicator-values-bulk-results.csv"; anchor.click(); URL.revokeObjectURL(url); };
